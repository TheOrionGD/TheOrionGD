...
async function getBucket() {
  ...
  try {
    ...
    if (!uri)          throw new Error('[mongo-assets] MONGODB_URI or CONNECTION_STRING is not set in .env');
...
export default function mongoAssetsPlugin() {
  return {
    ...
    configureServer(server) {
      // ── Windows crash guard ────────────────────────────────────────────────
      // On Windows, an unhandled async rejection from the MongoDB client kills
      // the Vite process with exit code 3221225786. We install a process-level
      // guard here (scoped to this plugin's startup) so any MongoDB error is
      // logged and swallowed instead of propagating to Vite's crash handler.
      const mongoRejectionGuard = (reason) => {
        if (
          reason?.message?.includes('mongo') ||
          reason?.message?.includes('MongoDB') ||
          reason?.message?.includes('ECONNREFUSED') ||
          reason?.message?.includes('ETIMEDOUT') ||
          reason?.message?.includes('mongo-assets')
        ) {
          console.error('\n[mongo-assets] ⚠️  Unhandled MongoDB rejection (caught by plugin guard):', reason?.message);
          // Do NOT rethrow — allow Vite to keep running
        }
      };
      process.on('unhandledRejection', mongoRejectionGuard);
      ...
      server.middlewares.use('/assets', async (req, res, next) => {
        try {
          ...
          // Skip Vite source/compiled files (e.g. .ts, .tsx, .js, .jsx, .css, sourcemaps) or HMR requests
          const cleanPath = requestedPath.split('?')[0];
          if (/\.(js|ts|tsx|jsx|css|map)$/i.test(cleanPath) || requestedPath.startsWith('/@') || req.headers['accept']?.includes('text/css')) {
            next();
            return;
          }
